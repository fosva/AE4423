...
class Node:
    ...
    def __init__(self, airpt, time):
        ...
        self.cargos = [np.zeros(AP)] # the cargos loaded to the aircraft when arriving at this airport
    ...
    def calc_profit(self, network, ac: Aircraft, dest_airpt):
        ...
        # if neither origin nor destination is the hub, the flight is infeasible
            ...
            # update route, times and cargos with the corresponding values for the current node
            route = [self.airpt] + dest.route
            times = [self.time] + dest.times
            cargos = [cargo] + dest_cargo + dest.cargos[1:]

            # Update block time and profit
